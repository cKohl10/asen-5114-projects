...
obs_poles(1) = 5 *real(Cl_poles(1));
obs_poles(2) = 5 *real(Cl_poles(2));
obs_poles(3) = 20 *real(Cl_poles(3)) + 0.8*imag(Cl_poles(3))*1j;
obs_poles(4) = 20 *real(Cl_poles(4)) + 0.8*imag(Cl_poles(4))*1j;
...
D_comb = D;
ss_comb = ss(A_comb,B_comb,C_comb,D_comb);
StepResponsePlot(ss_comb, 'Step Response');
