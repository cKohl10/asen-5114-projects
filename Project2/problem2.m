...
Cl_bandwidth = bandwidth(Cl_Tf,-3.1)
...
function x_opt = optimize_bandwidth()
    ...
    lb = [-1000, 0.5,-100,-10];   % Avoid poles too close to jω
...
function J = pole_bandwidth_cost(x, A, B, C)
    ...
    try
       % Place poles and compute closed-loop TF
        ...
        violation = mag_u2r > gain_limit;
        excess = mag_u2r - gain_limit;
        excess(~violation) = 0;  
        input_penalty = trapz(w, excess.^2);

        % Final cost: bandwidth deviation + notch penalty
        J = 100*(bw - 2*pi)^2 + 4*notch_penalty + 0.001*input_penalty;
